...
//changed this at various times between 10 - 200
const BATCH_SIZE = 10;


//changed this at various times between 20 - 2000
...
model.add(tf.layers.conv2d({
  ...
  kernelSize: 5,
  filters: 16, //changed filters to 8,32 for experiments
  strides: 1,
  activation: 'relu6', // changed from relu for testing
  kernelInitializer: 'varianceScaling'
}));

model.add(tf.layers.maxPooling2d({
  poolSize: [2, 2], // tried [3,3] as well
  strides: [2, 2]
}));

model.add(tf.layers.conv2d({
  kernelSize: 5,
  filters: 20, //changed filters to 8,16,32 for experiments
  strides: 1,
  activation: 'relu6',// changed from relu for testing
  kernelInitializer: 'varianceScaling'
}));
...
//tried to add a thrid ConvNet but it did not work very well 
/*model.add(tf.layers.conv2d({
  kernelSize: 5,
  filters: 20,
  strides: 1,
